...
function toneClass(value: number, good: number, warning: number, inverse = false) {
  if (inverse) {
...
function MiniCard({
  ...
  return (
    <article className="rounded-[12px] bg-[#2b2b2f99] p-3">
      <p className={`text-[20px] font-bold leading-none ${tone ?? "text-[#f7f1e6]"}`}>
        {value}
      </p>
      <p className="mt-2 text-[10px] font-normal uppercase tracking-[0.08em] text-[#f7f1e680]">
        {label}
      </p>
    </article>
  );
  ...
  return (
    <div>
      <div className="mb-1 flex items-center justify-between text-[12px]">
        <span className="text-[#f7f1e6cc]">{label}</span>
  ...
  useEffect(() => {
    ...
    async function loadAiHealth() {
      try {
        ...
        if (isActive) setError("AI performance is unavailable.");
      } finally {
  ...
  if (loading) {
    return (
      <div className="space-y-3 pb-24">
        <div className="h-12 rounded-[12px] bg-[#2b2b2f99]" />
        <div className="grid grid-cols-3 gap-2">
          <div className="h-24 rounded-[12px] bg-[#2b2b2f99]" />
          <div className="h-24 rounded-[12px] bg-[#2b2b2f99]" />
          <div className="h-24 rounded-[12px] bg-[#2b2b2f99]" />
        </div>
    ...
    <div className="space-y-5 pb-24">
      ...
      <section className="flex items-center justify-between rounded-[12px] bg-[#2b2b2fcc] px-4 py-3">
        {[
          ...
          <div className="flex items-center gap-2" key={label}>
            ...
            <span className="text-[11px] text-[#f7f1e699]">{label}</span>
          </div>
        ))}
      </section>
      ...
      <section>
        ...
        <div className="grid grid-cols-3 gap-2">
          ...
          <MiniCard
            ...
            tone={toneClass(payload.cache.avg_cached_response_ms, 200, 500, true)}
          />
          <MiniCard
            label="Live API"
            value={`${(payload.cache.avg_live_response_ms / 1000).toFixed(1)}s`}
            tone={toneClass(payload.cache.avg_live_response_ms / 1000, 2, 5, true)}
          />
        </div>
        <div className="mt-3 space-y-3 rounded-[12px] bg-[#2b2b2f99] p-4">
          <CoverageRow label="Simplify coverage" value={payload.cache.coverage.simplify_pct} />
          <CoverageRow label="Expand coverage" value={payload.cache.coverage.expand_pct} />
          <CoverageRow label="Slower coverage" value={payload.cache.coverage.slower_pct} />
        </div>
      </section>
      ...
      <section className="grid grid-cols-3 gap-2">
        ...
        <MiniCard
          ...
          tone={payload.stats.failed_transformations > 0 ? "text-[#c0392b]" : "text-[#7ab87a]"}
        />
      </section>
      ...
      <section className="rounded-[12px] bg-[#2b2b2f99] p-4">
        ...
        {(["simplify", "expand", "slower", "speed_up"] as const).map((key) => (
          <div className="flex items-center justify-between py-2 text-[13px]" key={key}>
            <span className="capitalize text-[#f7f1e6cc]">{key.replace("_", " ")}</span>
            <span className="text-[#f7f1e699]">
      ...
      <section className="grid grid-cols-2 gap-2">
        ...
        <MiniCard label="ESL share" value={`${payload.esl.esl_pct_of_total}%`} />
        <MiniCard label="Image success" value={`${payload.images.image_fetch_success_rate}%`} />
        <MiniCard label="AI cost today" value={`NGN ${payload.cost.cost_today_ngn}`} />
      </section>
      ...
      <section className="rounded-[12px] bg-[#2b2b2f99] p-4">
        ...
        {payload.errors.length ? (
          ...
          <p className="mt-3 text-center text-[13px] text-[#7ab87a]">No AI errors</p>
        )}
      </section>
    </div>
  );
