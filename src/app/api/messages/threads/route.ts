...
export async function GET() {
  try {
    ...
    const backendRes = await fetch(`${API_BASE_URL}/messages/threads`, {
      method: "GET",
      headers: {
    ...
    if (!backendRes.ok) {
      return NextResponse.json(
        { detail: buildErrorDetail(data, "Could not fetch message threads.") },
    ...
    return NextResponse.json(
      { detail: "Could not fetch message threads." },
      { status: 500 },
    );
  }
}
