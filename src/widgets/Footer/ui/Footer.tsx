...
export function Footer() {
    ...
    return (
        <div className="border-t text-sm xl:py-15 py-13 xl:px-20 px-12 border-graphite-5 flex flex-col xl:gap-16 gap-12">
            <div className="flex items-center justify-between">
                ...
                <ul className="grid grid-cols-2 gap-x-12 gap-y-3">
                    {FooterLinks.map((link) => {
                        return (
                            <li className="capitalize font-medium xl:text-base text-sm flex-1" key={link.name}> <a href={link.url}> {link.name} </a> </li>
                        )
                    })}
                </ul>
            </div>
