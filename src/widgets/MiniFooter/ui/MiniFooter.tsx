...
export function MiniFooter({ speaker, logIn }: { speaker?: boolean, logIn?: boolean }) {
    return (
        <nav className={`px-6 sm:px-12 bg-parchment py-4 flex ${(speaker || logIn) ? 'justify-between' : 'justify-center'} items-center`}>
            ...
            {logIn && <a href="/login" className="font-medium text-indigo">Already have an account? Log in</a>}
        </nav>
    )
}